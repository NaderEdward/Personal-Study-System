...
def get_assessments(subject, info=None, date=None):
    if info:
        quizes = reversed(db.execute("SELECT * FROM quizes WHERE info = ?", info))
        exams = reversed(db.execute("SELECT * FROM exams WHERE info = ?", info))
        return quizes, exams
    elif date:
...
def add_assessment(assessment_type, subject, grade, mistakes, info):
    ...
    if assessment_type == "quiz":
        db.execute("INSERT INTO quizes (subject, grade, percent, mistakes, info, date) VALUES (?,?,?,?,?,?)", subject, grade, percent, mistakes, info, date_)
    else:
        db.execute("INSERT INTO exams (subject, grade, percent, mistakes, info, date) VALUES (?,?,?,?,?,?)", subject, grade, percent, mistakes, info, date_)
    return 1
    ###
...
def add_subject(name, subjects_list):
    if name:
        if name in subjects_list:
            return False
        db.execute("INSERT INTO subjects (name, n_topics, topics_file) VALUES (?,?,?)", name.title(), 0, f"topics/__{name.upper()}__.csv")
        with open(f"topics/__{name.upper()}__.csv", "w") as topic_file_csv:
            writer = csv.DictWriter(topic_file_csv, fieldnames=["topic","difficulty","strength"])
            writer.writeheader()
...
def add_topic(subject, topic, difficulty, strength):
    if os.path.exists(f"topics/__{subject.upper()}__.csv"):
        with open(f"topics/__{subject.upper()}__.csv", "a") as topic_file_csv:
            appender = csv.DictWriter(topic_file_csv, fieldnames=["topic","difficulty","strength"])
            appender.writerow({"topic": topic,"difficulty": difficulty,"strength": strength})
    else:
        with open(f"topics/__{subject.upper()}__.csv", "w") as topic_file_csv:
            writer = csv.DictWriter(topic_file_csv, fieldnames=["topic","difficulty","strength"])
            writer.writeheader()
            writer.writerow({"topic": topic,"difficulty": difficulty,"strength": strength})
        return topic
    ###

# Remove topic
def remove_topic(topics_file, topic):
    if topic and topics_file:    
        with open(f"topics/__{topics_file}__.csv", "r") as topics_file_csv:
            ...
            topics = [topic for topic in reader if topic["topic"]!= topic]
        with open(f"topics/__{topics_file}__.csv", "w") as topics_file_csv:
            writer = csv.DictWriter(topics_file_csv, fieldnames=["topic","difficulty","strength"])
            writer.writeheader()
...
def get_fact():
    ...
    response = requests.get(api_url, headers={'X-Api-Key': 'pH7JDdWIkIYMMRmDM7r05Q==O0DXEgZuUgQXRe0k'})
...
def get_todo_items():
    ...
    with open("docs/todo.csv", "r") as todo_file_csv:
        ...
        for row in reader:
            items.append({"item": row["item"].capitalize(), "importance": row["importance"]})
    if items:
...
def classify_percentages(percentages):
    ...
    counts = [classifications["Excellent"], classifications["Good"], classifications["Average"], classifications["Below Average"]]
    ...
    for subject in subjects_list:
        if subject not in subjects:
            with open(f"topics/__{subject.upper()}__.csv", "w") as topic_file_csv:
                writer = csv.DictWriter(topic_file_csv, fieldnames=["topic","difficulty","strength"])
                writer.writeheader()
            db.execute("INSERT INTO subjects (name, n_topics, topics_file) VALUES (?,?,?)", subject,0,f"topics/__{subject.upper()}__.csv")
